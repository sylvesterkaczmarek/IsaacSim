...
- Fix `cross()` to return `np.ndarray` instead of Python list (6035994)
- Fix `distance_metrics` validators to raise `ValueError` instead of only logging errors (6035336)
- Fix `rotational_distance_single_axis()` NaN for zero-magnitude axis vector (6035351)
- Fix `recompute_extents()` crash on mesh with no authored points (6035377)
- Fix `recompute_extents()` log level from `log_info` to `log_warn` for non-Boundable prims (6036016)
- Fix `compute_aabb()` crash on nonexistent prim path (6036011)
- Fix `compute_aabb`/`compute_combined_aabb` shadowing Python builtin `range` (6036008)
- Fix `compute_combined_aabb()` crash on empty prim_paths list (6020206)
- Fix `get_prim_attribute_names()` misleading error when no USD stage is open (6036161)
- Fix `is_prim_no_delete()` returning None instead of False when metadata absent (6036179)
- Fix `is_prim_hidden_in_stage()` returning None instead of False when metadata absent (6036184)
- Fix `get_first_matching_child_prim()` dead variable and O(N²) BFS concat (6035685)
- Fix `get_all_matching_child_prims()` O(N²) list concatenation in BFS/DFS traversal (6035706)
- Fix `query_parent_path()` crash with TypeError on root path (6036174)
- Fix `set_prim_attribute_value()` eval() security risk replaced with getattr resolution (6035670)
- Fix `define_prim()` raising bare `Exception` instead of `ValueError` (6035674)
- Fix `create_prim()` crash for non-schema attribute keys (6036194)
- Fix `simulate_async()` running fewer frames than requested due to int truncation (6036005)
- Fix `print_stage_prim_paths()` writing to stdout via bare print (6035651)
- Fix `save_stage()` destroying existing USD file before content committed (6036205)
- Fix `add_reference_to_stage()` silently returning zombie prim for nonexistent USD file (6036198)
- Fix `lookat_to_quatf()` non-unit quaternion when camera and target are identical (6035395)
- Fix `get_transform_with_normalized_rotation()` NaN when rotation column has zero magnitude (6035463)
- Fix `SpringDamperFollower.update()` divide-by-zero when mass is zero (6035418)
- Fix `find_unique_string_name()` redundant `is_unique_fn` call (6035961)
- Fix `get_type()` duplicate `elif dtype=="bool"` dead code branch in warp/tensor.py (6035964)
- Fix `move_data()` in warp/tensor.py returning None for unsupported input types (6035640)
- Fix `set_intrinsics_matrix()` debug print statements to stdout (6036032)
- Fix `set_intrinsics_matrix()` dead `if prim is None` guard replaced with proper validation (6036034)
- Fix `_get_world_pose_transform_w_scale()` RecursionError on deep hierarchies (6036047)
- Fix `get_mesh_vertices_relative_to()` crash on mesh with no authored points (6035404)
- Fix `get_extension_path_from_name()` returning empty string for unloaded extensions (6036029)
- Fix `find_all_articulation_base_paths()` uninitialized variable `r` (6036210)
- Fix `find_all_articulation_base_paths()` IndexError on different-depth body paths (6036214)
- Fix `transpose_2d()` (numpy and torch) corrupting batch dimensions for 3D+ inputs (6035758)
