...
class MotionPolicy(WorldInterface):
    ...
    The MotionPolicy interface inherits from the WorldInterface class. A MotionPolicy can be passed to an
    ArticulationMotionPolicy to streamline moving the simulated robot.
    """
    ...
    def set_robot_base_pose(self, robot_translation: np.array, robot_orientation: np.array) -> None:
        """Updates the position of the robot base.
    ...
    def compute_joint_targets(
        ...
        """Computes position and velocity targets for the next frame given the current robot state.
    ...
    def get_active_joints(self) -> list[str]:
        ...
        Returns:
            Names of active joints. The order of joints in this list determines the order in which a MotionPolicy expects
            joint states to be specified in functions like compute_joint_targets(active_joint_positions,...).
        """
        return []
    ...
    def get_watched_joints(self) -> list[str]:
        ...
        E.g., a MotionPolicy may control a robot arm on a mobile robot. The joint states in the rest of the robot directly
        affect the position of the arm, but they are not actively controlled by this MotionPolicy.

        Returns:
            Names of joints that are being watched by this MotionPolicy. The order of joints in this list determines the
            order in which a MotionPolicy expects joint states to be specified in functions like
            compute_joint_targets(...,watched_joint_positions,...).
        """
        return []

    def set_cspace_target(self, active_joint_targets: np.array) -> None:
        """Sets the configuration space target for the robot.
