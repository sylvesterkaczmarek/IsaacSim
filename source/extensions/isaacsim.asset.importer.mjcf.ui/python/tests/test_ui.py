...
class TestImporterUI(MenuUITestCase):
    ...
    async def test_import_ant_adds_reference_to_current_stage(self) -> None:
        """Add the imported ant USD as a reference to the current stage."""
        ...
        extension.build_new_options([self._mjcf_path])
        state = extension._per_file_state[mjcf_ui_extension._normalize_path_key(self._mjcf_path)]
        ...
        output_path = extension._start_import(self._mjcf_path)
