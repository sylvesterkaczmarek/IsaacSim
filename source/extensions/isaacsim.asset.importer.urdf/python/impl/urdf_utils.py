...
def _rewrite_relative_mesh_paths_to_absolute(
    ...
    for mesh in tree.getroot().iter("mesh"):
        ...
        if os.path.isabs(stripped):
            continue
        mesh.set("filename", os.path.normpath(os.path.join(reference_dir, stripped)))
        changed = True
    if changed:
...
def merge_fixed_joints(urdf_path: str, output_path: str) -> str:
    ...
    while True:
        ...
        if parent_elem is None or child_elem is None:
            _logger.warning(
                "Fixed joint '%s' in %s is missing <parent> and/or <child> element; "
                "dropping the joint without merging links.",
                joint_name,
                urdf_path,
            )
            root.remove(joint)
        ...
        if parent_link_name is None or child_link_name is None:
            _logger.warning(
                "Fixed joint '%s' in %s has parent/child element with no 'link' "
                "attribute (parent=%r, child=%r); dropping the joint without merging.",
                joint_name,
                urdf_path,
                parent_link_name,
                child_link_name,
        ...
        if parent_link_name == child_link_name:
            _logger.error(
                "Fixed joint '%s' in %s is a self-loop (parent == child == '%s'); "
                "dropping the joint without merging to avoid removing the link.",
                joint_name,
                urdf_path,
                parent_link_name,
            )
        ...
        if parent_link_elem is None or child_link_elem is None:
            _logger.error(
                "Fixed joint '%s' in %s references missing link(s) "
                "(parent='%s' found=%s, child='%s' found=%s); dropping the joint "
                "without merging. Downstream joints may be left dangling.",
                joint_name,
                urdf_path,
                parent_link_name,
                parent_link_elem is not None,
                child_link_name,
                child_link_elem is not None,
            )
            root.remove(joint)
            continue
        ...
        for sibling in root:
            ...
            if f'"{child_link_name}"' in serialized or f"'{child_link_name}'" in serialized:
                _logger.warning(
                    "URDF extension element <%s> in %s references child link '%s' "
                    "being removed by merge_fixed_joints; the reference will be "
                    "left dangling.",
                    sibling.tag,
                    urdf_path,
                    child_link_name,
        ...
        for other_joint in root.findall("joint"):
            ...
            if other_parent is not None and other_parent.get("link") == child_link_name:
                other_parent.set("link", parent_link_name)
