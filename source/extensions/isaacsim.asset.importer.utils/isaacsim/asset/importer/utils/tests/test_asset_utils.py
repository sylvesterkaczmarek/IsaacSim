...
class TestAssetUtils(omni.kit.test.AsyncTestCase):
    ...
    async def test_detect_fixed_base_internal_revolute(self) -> None:
        """A revolute joint between two rigid bodies is NOT a world anchor."""
        stage = _make_stage()
        root = _add_rigid_link(stage, "/robot/base")
        _add_rigid_link(stage, "/robot/link1")
    ...
    async def test_detect_fixed_base_revolute_world_anchor(self) -> None:
        """A revolute joint with body0 unset (world) should be detected as fixed base."""
        stage = _make_stage()
        root = _add_rigid_link(stage, "/robot/base")
        rj = _add_revolute_joint(stage, "/robot/rj", None, "/robot/base")
        self.assertTrue(asset_utils._detect_fixed_base(stage, root, [rj]))

    async def test_detect_fixed_base_revolute_to_non_rigid(self) -> None:
        """A revolute joint to a non-rigid Xform should be detected as fixed base."""
        stage = _make_stage()
        root = _add_rigid_link(stage, "/robot/base")
        rj = _add_revolute_joint(stage, "/robot/rj", "/robot", "/robot/base")
        self.assertTrue(asset_utils._detect_fixed_base(stage, root, [rj]))

    async def test_detect_fixed_base_body0_only_ignored(self) -> None:
        """Ignore body0-only joints (body1 empty)."""
        stage = _make_stage()
        root = _add_rigid_link(stage, "/robot/base")
        fj = UsdPhysics.FixedJoint.Define(stage, "/robot/fj")
        fj.CreateBody0Rel().SetTargets([root.GetPath()])
        self.assertFalse(asset_utils._detect_fixed_base(stage, root, [fj.GetPrim()]))

    async def test_detect_fixed_base_anchor_on_non_root_link(self) -> None:
        """Anchoring any link of the articulation fixes the robot, not only the root."""
        stage = _make_stage()
        root = _add_rigid_link(stage, "/robot/base")
        _add_rigid_link(stage, "/robot/link1")
        rj = _add_revolute_joint(stage, "/robot/rj", "/robot/base", "/robot/link1")
        fj_prim = _add_fixed_joint(stage, "/robot/fj", "/robot", "/robot/link1")
        self.assertTrue(asset_utils._detect_fixed_base(stage, root, [rj, fj_prim]))

    async def test_detect_fixed_base_ignores_anchor_on_other_articulation(self) -> None:
        """A world anchor on an unrelated rigid body must not count as a fixed base."""
        stage = _make_stage()
        root = _add_rigid_link(stage, "/robot/base")
        _add_rigid_link(stage, "/robot/prop")
        fj_prim = _add_fixed_joint(stage, "/robot/prop_fj", "/robot", "/robot/prop")
        self.assertFalse(asset_utils._detect_fixed_base(stage, root, [fj_prim]))
    ...
    async def test_fix_base_creates_joint(self) -> None:
        ...
        self.assertTrue(fix_joint_prim.HasAPI(UsdPhysics.ArticulationRootAPI))

    async def test_fix_base_skips_when_already_fixed(self) -> None:
        """Leave existing fixed-base articulation authoring unchanged."""
        stage = _make_stage()
        geometry = UsdGeom.Xform.Define(stage, "/robot/Geometry").GetPrim()
        link = _add_rigid_link(stage, "/robot/Geometry/base")
        UsdPhysics.ArticulationRootAPI.Apply(link)
        _add_fixed_joint(stage, "/robot/Physics/root_joint", None, "/robot/Geometry/base")

        asset_utils.apply_fix_base(stage)

        fix_joint_prim = stage.GetPrimAtPath("/robot/fix_base_joint")
        self.assertFalse(fix_joint_prim.IsValid())
        self.assertTrue(link.HasAPI(UsdPhysics.ArticulationRootAPI))
        self.assertFalse(geometry.HasAPI(UsdPhysics.ArticulationRootAPI))
        self.assertFalse(stage.GetPrimAtPath("/robot/Physics/root_joint").HasAPI(UsdPhysics.ArticulationRootAPI))

    async def test_fix_base_skips_when_revolute_world_anchor(self) -> None:
        """Do not add a FixedJoint when any joint type already anchors the robot."""
        stage = _make_stage()
        _add_rigid_link(stage, "/robot/base")
        _add_revolute_joint(stage, "/robot/world_rj", None, "/robot/base")

        asset_utils.apply_fix_base(stage)

        self.assertFalse(stage.GetPrimAtPath("/robot/fix_base_joint").IsValid())
        self.assertTrue(stage.GetPrimAtPath("/robot/world_rj").IsValid())
    ...
    async def test_floating_base_restores_articulation_root_to_root_link(self) -> None:
        """Move fixed-joint articulation authoring back to the root link before removing the joint."""
        stage = _make_stage()
        link = _add_rigid_link(stage, "/robot/base")
        UsdPhysics.ArticulationRootAPI.Apply(link)
        link.ApplyAPI("NewtonArticulationRootAPI")
        link.CreateAttribute("newton:selfCollisionEnabled", Sdf.ValueTypeNames.Bool).Set(False)
        link.CreateAttribute("newton:jointsAddMobility", Sdf.ValueTypeNames.Bool).Set(True)
        ...
        self.assertFalse(stage.GetPrimAtPath("/robot/fix_base_joint").IsValid())
        self.assertTrue(link.HasAPI(UsdPhysics.ArticulationRootAPI))
        self.assertTrue(link.HasAPI("NewtonArticulationRootAPI"))
        self.assertFalse(link.GetAttribute("newton:selfCollisionEnabled").Get())
        self.assertTrue(link.GetAttribute("newton:jointsAddMobility").Get())
    ...
    async def test_floating_base_ignores_body0_only_joint(self) -> None:
        """Leave body0-only joints in place."""
        stage = _make_stage()
        root = _add_rigid_link(stage, "/robot/base")
        ...
        self.assertTrue(stage.GetPrimAtPath("/robot/world_fj").IsValid())

    async def test_floating_base_removes_anchor_on_non_root_link(self) -> None:
        """A world anchor on a child link must be removed to free the articulation."""
        stage = _make_stage()
        _add_rigid_link(stage, "/robot/base")
        _add_rigid_link(stage, "/robot/link1")
        _add_revolute_joint(stage, "/robot/rj", "/robot/base", "/robot/link1")
        _add_fixed_joint(stage, "/robot/link_fj", "/robot", "/robot/link1")

        asset_utils.apply_floating_base(stage)

        self.assertFalse(stage.GetPrimAtPath("/robot/link_fj").IsValid())
    ...
    async def test_floating_base_preserves_internal_joints(self) -> None:
        """Joints between two rigid bodies must be left alone."""
        stage = _make_stage()
        _add_rigid_link(stage, "/robot/base")
        _add_rigid_link(stage, "/robot/link1")
        _add_revolute_joint(stage, "/robot/rj", "/robot/base", "/robot/link1")
    ...
    async def test_floating_base_removes_revolute_world_anchor(self) -> None:
        """A revolute joint with body0 unset (world) must be removed."""
        stage = _make_stage()
        _add_rigid_link(stage, "/robot/base")
        _add_revolute_joint(stage, "/robot/world_rj", None, "/robot/base")
        ...
        self.assertFalse(stage.GetPrimAtPath("/robot/world_rj").IsValid())
    ...
    async def test_floating_base_no_anchor_present(self) -> None:
        """When there's no world-anchoring joint, the stage stays untouched."""
        stage = _make_stage()
        _add_rigid_link(stage, "/robot/base")
        _add_rigid_link(stage, "/robot/link1")
        _add_revolute_joint(stage, "/robot/rj", "/robot/base", "/robot/link1")
    ...
    async def test_fix_base_relocates_articulation_root_to_fixed_joint(self) -> None:
        """Move a new fixed-base articulation root to its fixed joint."""
        stage = _make_stage()
        geometry = UsdGeom.Xform.Define(stage, "/robot/Geometry").GetPrim()
        UsdGeom.Xformable(geometry).AddScaleOp().Set(Gf.Vec3f(0.5, 0.5, 0.5))
        link = _add_rigid_link(stage, "/robot/Geometry/base")
        UsdPhysics.ArticulationRootAPI.Apply(link)
        link.ApplyAPI("NewtonArticulationRootAPI")
        link.CreateAttribute("newton:selfCollisionEnabled", Sdf.ValueTypeNames.Bool).Set(False)
        ...
        fixed_joint = stage.GetPrimAtPath("/robot/fix_base_joint")
        self.assertTrue(fixed_joint.HasAPI(UsdPhysics.ArticulationRootAPI))
        self.assertTrue(fixed_joint.HasAPI("NewtonArticulationRootAPI"))
        self.assertFalse(geometry.HasAPI(UsdPhysics.ArticulationRootAPI))
        self.assertFalse(link.HasAPI(UsdPhysics.ArticulationRootAPI))
        self.assertFalse(link.HasAPI("NewtonArticulationRootAPI"))
        self.assertFalse(link.GetAttribute("newton:selfCollisionEnabled").IsValid())
        self.assertFalse(link.GetAttribute("newton:jointsAddMobility").IsValid())
        self.assertFalse(fixed_joint.GetAttribute("newton:selfCollisionEnabled").Get())
        self.assertTrue(fixed_joint.GetAttribute("newton:jointsAddMobility").Get())

    # -- fix_articulation_root_for_fixed_base --------------------------------

    async def test_art_root_moved_to_fixed_joint(self) -> None:
        """ArticulationRootAPI should be relocated from the rigid body to its fixed joint."""
        stage = _make_stage()
        link = _add_rigid_link(stage, "/robot/base")
        fixed_joint = _add_fixed_joint(stage, "/robot/root_joint", None, "/robot/base")
        UsdPhysics.ArticulationRootAPI.Apply(link)
    ...
    async def test_art_root_fixed_joint_already_has_api(self) -> None:
        """Remove a duplicate rigid-body articulation root when its fixed joint already has one."""
        stage = _make_stage()
        link = _add_rigid_link(stage, "/robot/base")
        UsdPhysics.ArticulationRootAPI.Apply(link)
        fixed_joint = _add_fixed_joint(stage, "/robot/root_joint", None, "/robot/base")
        UsdPhysics.ArticulationRootAPI.Apply(fixed_joint)
    ...
    async def test_art_root_multiple(self) -> None:
        """Multiple articulation roots should be relocated to their fixed joints."""
        stage = _make_stage()
        link_a = _add_rigid_link(stage, "/robot/a")
        link_b = _add_rigid_link(stage, "/robot/b")
        UsdPhysics.ArticulationRootAPI.Apply(link_a)
        UsdPhysics.ArticulationRootAPI.Apply(link_b)
        fixed_joint_a = _add_fixed_joint(stage, "/robot/root_joint_a", None, "/robot/a")
        fixed_joint_b = _add_fixed_joint(stage, "/robot/root_joint_b", None, "/robot/b")

        count = asset_utils.fix_articulation_root_for_fixed_base(stage)
        self.assertEqual(count, 2)
        self.assertTrue(fixed_joint_a.HasAPI(UsdPhysics.ArticulationRootAPI))
        self.assertTrue(fixed_joint_b.HasAPI(UsdPhysics.ArticulationRootAPI))
