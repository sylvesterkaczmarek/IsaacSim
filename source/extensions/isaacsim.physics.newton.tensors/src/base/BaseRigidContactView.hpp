...
public:
     ...
     * @param[out] outPaths Receives absolute USD prim paths. Static contacts resolve
     * to the specific contacted shape rather than a synthetic world-body path.
     */
...
protected:
    ...
    /// Rebuilds sensor/filter body index maps from the current Newton model body labels.
    void _rebuildBodyMaps();
    ...
    mutable std::vector<int> m_hostBodySensorMap; ///< [numBodies] → sensor index, -1 if not a sensor.
    mutable std::vector<int> m_hostBodyFilterMap; ///< [sensorCount * numBodies] → filter index per sensor, -1 if no
                                                  ///< match.
    std::vector<std::string> m_shapeLabels; ///< Shape labels from the Newton model.
    std::vector<int> m_shapeBodyArr; ///< Shape-to-body index mapping from the Newton model.
