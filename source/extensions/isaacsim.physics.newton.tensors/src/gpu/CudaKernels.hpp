...
///
/// Local inputs are rotated into the world frame. When an application position
/// is supplied, the resulting ``(position - centerOfMass) x force`` moment is
/// added to the direct torque.
///
/// @param force Device force input, or ``nullptr`` when ``hasForce`` is false.
/// @param torque Device torque input, or ``nullptr`` when ``hasTorque`` is false.
/// @param position Device application-position input, or ``nullptr`` when ``hasPosition`` is false.
/// @param bodyForce Device destination spatial-force buffer, accumulated via atomic add.
/// @param articulationIndices Optional device articulation/body-view index array.
/// @param linkMapping Device mapping from view link slots to model body indices.
/// @param bodyTransforms Device model body transforms.
/// @param bodyCentersOfMass Device model body center-of-mass offsets.
/// @param numArticulations Number of selected articulations or rigid bodies.
/// @param maxLinks Maximum links per articulation; use one for rigid bodies.
/// @param isGlobal Whether input vectors and positions are in the world frame.
/// @param hasForce Whether ``force`` is present.
/// @param hasTorque Whether ``torque`` is present.
/// @param hasPosition Whether ``position`` is present.
/// @param stream CUDA stream.
/// @return ``true`` if the launch succeeded.
...
bool launchFrictionData(const int* contactCount,
                        const int* shape0,
                        const int* shape1,
                        const float* point0,
                        const float* point1,
