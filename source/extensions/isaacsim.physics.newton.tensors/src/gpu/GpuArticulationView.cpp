                                                          ...
                                                          bool isGlobal)
{
    ...
    if (srcPositionTensor && srcPositionTensor->data &&
        !validateFloat32TensorAnyDevice(srcPositionTensor, size_t(m_count) * m_maxLinks * 3u, "position", __FUNCTION__))
        return false;
    ...
    const size_t inputFloats = size_t(m_count) * m_maxLinks * 3u;
    ...
    return launchFusedLinkWrenchAdd(force, torque, position, m_cachedBodyF, indices.ptr, m_deviceLinkFlatIndices,
                                    reinterpret_cast<const wp::transform*>(m_cachedBodyQ),
                                    reinterpret_cast<const wp::vec3*>(m_cachedBodyCenterOfMass), indices.count,
                                    m_maxLinks, isGlobal, hasForce, hasTorque, hasPosition);
}
