...
"""Frozen policy spec for the ANYmal-C flat-terrain locomotion example.

The 48-wide locomotion observation and 12-wide joint-position action derive from the selected
engine's exported IO descriptor, including its 0.5 action scale. Joint names bind literally, so
each engine deploys the asset its policy was trained against.

Both env configs declare the ANYdrive-3 LSTM actuator network on every joint, which the runner
builds from the config's actuator model specs, so the spec carries no actuator fields. The spawn
orientation is pinned to identity because the hosted env configs mix quaternion conventions.
"""
...
__all__ = ["get_anymal_spec"]


@functools.cache
def _get_default_anymal_spec() -> PolicySpec:
    """Build the cached default ANYmal-C flat-terrain policy spec.
    ...
    policy_dir = f"{assets_root_path}/Isaac/Samples/Policies/Anymal_Policies"
    return PolicySpec(
        name="anymal_flat_terrain",
        engines={
            "physx": PolicyArtifact.from_files(
                f"{policy_dir}/anymal_policy.pt",
                f"{policy_dir}/anymal_env.yaml",
                f"{policy_dir}/anymal_IO_descriptors.yaml",
                model_sha256="8b2d6277b7315ef601482d809df77d8086c9a6247cdf20f7770927f8d2c31a6f",
            ),
            "newton": PolicyArtifact.from_files(
                f"{policy_dir}/anymal_newton_policy.pt",
                f"{policy_dir}/anymal_newton_env.yaml",
                f"{policy_dir}/anymal_newton_IO_descriptors.yaml",
                model_sha256="7f2efe7290a3038e0f9cdd37249bf540cbad18b53daf7639f365617d8c1f3db8",
            ),
        },
        usd_path=f"{assets_root_path}/Isaac/IsaacLab/Robots/ANYbotics/ANYmal-C/anymal_c.usd",
...
def get_anymal_spec(
    *,
    ...
    """Return the bundled ANYmal-C spec; omitted :class:`PolicySpec` fields keep their defaults.
