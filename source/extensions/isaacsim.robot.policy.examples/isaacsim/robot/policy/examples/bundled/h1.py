# SPDX-FileCopyrightText: Copyright (c) 2026 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# SPDX-License-Identifier: Apache-2.0
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
# http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""Frozen policy spec for the Unitree H1 flat-terrain locomotion example.

The 69-wide locomotion observation and 19-wide joint-position action derive from the selected
engine's exported IO descriptor, including its 0.5 action scale.

The robot USD is engine-keyed because joint names bind literally: Newton deploys the
``h1_minimal.usd`` asset its policy was trained against, since the Menagerie import uniquifies
the trained ``torso`` joint to ``torso_1`` and no longer matches the export. The spawn pose is
pinned to the 1.05 m stand height at identity orientation.
"""

from __future__ import annotations

import functools
from collections.abc import Callable, Mapping
from dataclasses import replace
from types import EllipsisType

from ..spec import PolicyArtifact, PolicySpec

__all__ = ["get_h1_spec"]


@functools.cache
def _get_default_h1_spec() -> PolicySpec:
    """Build the cached default H1 flat-terrain policy spec.

    Returns:
        The resulting :class:`PolicySpec`.
    """
    from isaacsim.storage.native import get_assets_root_path

    assets_root_path = get_assets_root_path()
    if not assets_root_path:
        raise RuntimeError("Could not resolve the assets root path; check the Isaac Sim asset configuration.")

    policy_dir = f"{assets_root_path}/Isaac/Samples/Policies/h1"
    return PolicySpec(
        name="h1_flat_terrain",
        engines={
            "physx": PolicyArtifact.from_files(
                f"{policy_dir}/physx_policy.pt",
                f"{policy_dir}/physx_env.yaml",
                f"{policy_dir}/h1_physx_IO_descriptors.yaml",
                model_sha256="a734d8ae13b8c3690138baf8d01d7d497f987616da2c53e00a43e0ceb31452d7",
            ),
            "newton": PolicyArtifact.from_files(
                f"{policy_dir}/newton_policy.pt",
                f"{policy_dir}/newton_env.yaml",
                f"{policy_dir}/h1_newton_IO_descriptors.yaml",
                model_sha256="606ff58cfb19f0b4ee87d9bd820b77935e7e4a0e459a6d61675236abf34d4140",
            ),
        },
        usd_path={
            "physx": f"{assets_root_path}/Isaac/Robots/Unitree/H1/h1.usd",
            "newton": f"{assets_root_path}/Isaac/IsaacLab/Robots/Unitree/H1/h1_minimal.usd",
        },
        default_spawn_orientation=(1.0, 0.0, 0.0, 0.0),
        default_spawn_position=(0.0, 0.0, 1.05),
    )


def get_h1_spec(
    *,
    engines: Mapping[str, PolicyArtifact] | EllipsisType = ...,
    usd_path: str | Mapping[str, str] | None | EllipsisType = ...,
    name: str | None | EllipsisType = ...,
    default_spawn_position: tuple[float, float, float] | None | EllipsisType = ...,
    default_spawn_orientation: tuple[float, float, float, float] | None | EllipsisType = ...,
    zero_targets_on_initialize: bool | EllipsisType = ...,
    binding: Callable | None | EllipsisType = ...,
) -> PolicySpec:
    """Return the bundled H1 spec; omitted :class:`PolicySpec` fields keep their defaults.

    Args:
        engines: Per-engine artifacts keyed by engine name.
        usd_path: Robot USD path, or an engine-keyed mapping of them.
        name: Diagnostic label used in runner error messages.
        default_spawn_position: World-frame spawn position used when the caller passes none.
        default_spawn_orientation: World-frame WXYZ spawn orientation for the same fallback.
        zero_targets_on_initialize: Reset to the default state and zero all targets on initialize.
        binding: Explicit binding hook used instead of descriptor derivation.

    Returns:
        The resulting :class:`PolicySpec`.
    """
    overrides = {field: value for field, value in locals().items() if value is not ...}
    spec = _get_default_h1_spec()
    return replace(spec, **overrides) if overrides else spec
