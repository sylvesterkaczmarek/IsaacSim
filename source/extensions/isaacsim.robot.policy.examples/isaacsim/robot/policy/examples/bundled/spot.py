...
"""Frozen policy spec for the Boston Dynamics Spot flat-terrain locomotion example.

The 48-wide locomotion observation and 12-wide joint-position action derive from the selected
engine's exported IO descriptor, including its 0.2 action scale. Joint names bind literally, so
...
__all__ = ["get_spot_spec"]


@functools.cache
def _get_default_spot_spec() -> PolicySpec:
    """Build the cached default Spot flat-terrain policy spec.
    ...
    policy_dir = f"{assets_root_path}/Isaac/Samples/Policies/Spot_Policies"
    return PolicySpec(
        name="spot_flat_terrain",
        engines={
            "physx": PolicyArtifact.from_files(
                f"{policy_dir}/physx_policy.pt",
                f"{policy_dir}/physx_env.yaml",
                f"{policy_dir}/spot_physx_IO_descriptors.yaml",
                model_sha256="37084fd7d48ffc23100b025b3ffc92bc00507a5be6f676b005781d579d834b8d",
            ),
            "newton": PolicyArtifact.from_files(
                f"{policy_dir}/newton_policy.pt",
                f"{policy_dir}/newton_env.yaml",
                f"{policy_dir}/spot_newton_IO_descriptors.yaml",
                model_sha256="dfbda8c741445338dc5279fdb1aa82f6ce3b71eb85fb0b8f1212534e5a9fdb51",
            ),
        },
        usd_path=f"{assets_root_path}/Isaac/Robots/BostonDynamics/spot/spot.usd",
...
def get_spot_spec(
    *,
    ...
    """Return the bundled Spot spec; omitted :class:`PolicySpec` fields keep their defaults.
