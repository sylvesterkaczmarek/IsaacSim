...
"""Robot Poser kinematic chain implementation for FK computation and USD application.
...
:mod:`isaacsim.robot.poser.math` provides the data structures and pure math
utilities used by the chain.
"""
...
# ---------------------------------------------------------------------------
# Kinematic-tree and zero-config utilities
# ---------------------------------------------------------------------------


def _find_tree_node(root: Any, target_path: str) -> Any:
    """Find a robot link tree node by prim path.
    ...
    Returns:
        Matching node, or None if no node matches.
    """
...
def _ancestors(node: Any) -> list[Any]:
    """Return the node and each ancestor through the tree root.

    Args:
        node: Tree node to start from.

    Returns:
        Nodes ordered from the input node through the tree root.
    """
    chain: list[Any] = []
    current = node
...
def _collect_chain_joints(start_node: Any, end_node: Any) -> list[tuple[Any, bool]]:
    """Collect joints along the unique tree path between two nodes.
    ...
    Returns:
        Joint prim and forward-traversal flag pairs in chain order.

    Raises:
        ValueError: If the nodes do not belong to the same tree.
    """
    start_ancestors = _ancestors(start_node)
    end_ancestors = _ancestors(end_node)
    start_ids = {id(node) for node in start_ancestors}

    lowest_common_ancestor = next((node for node in end_ancestors if id(node) in start_ids), None)
    if lowest_common_ancestor is None:
        raise ValueError("Start and end nodes are not connected in the kinematic tree")

    backward: list[tuple[Any, bool]] = []
    current = start_node
    while current is not lowest_common_ancestor:
        if current._joint_to_parent is not None:
            backward.append((current._joint_to_parent, False))
        current = current.parent

    forward: list[tuple[Any, bool]] = []
    current = end_node
    while current is not lowest_common_ancestor:
        if current._joint_to_parent is not None:
            forward.append((current._joint_to_parent, True))
        current = current.parent
    forward.reverse()
...
def _compute_zero_config_poses(tree_root: Any) -> dict[str, Any]:
    """Compute body world transforms at the joint-zero configuration.

    Args:
        tree_root: Root node returned by ``GenerateRobotLinkTree``.

    Returns:
        Mapping from body prim paths to zero-configuration world transforms.
    """
    import pxr
        ...
        for child in node.children:
            ...
            if joint_prim is not None:
                ...
                if local0 is not None and local1 is not None:
                    zero_world[child_path] = (
                        pxr.Gf.Matrix4d(local1).GetInverse() * pxr.Gf.Matrix4d(local0) * parent_zero
                    )
                else:
