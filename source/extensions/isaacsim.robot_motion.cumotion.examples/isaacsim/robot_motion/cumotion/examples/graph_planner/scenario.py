...
class FrankaGraphPlannerExample:
    ...
    def get_controlled_joint_names(self) -> list[str]:
        """Get the names of joints the planner controls.

        Returns:
            Joint names controlled by the planner, or an empty list if the robot config is not loaded.
        """
    ...
    def _path_to_trajectory(self, path: Any, robot_config: Any) -> Any:
        ...
        Args:
            path: Planned path returned by the graph-based motion planner.
            robot_config: cuMotion robot configuration used to map active joints.

        Returns:
            Minimal-time joint trajectory for the planned path.
        """
