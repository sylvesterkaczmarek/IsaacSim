...
def generate_distinct_colors(n: int) -> list[list[int]]:
    ...
    # The first joint's auto-generated colour is a hard-to-read pure blue.
    # Override just that group with a brighter, more legible blue (#3B82F6) plus
    # two lighter variants for the secondary/observed series.  Values are stored
    # in omni.ui ABGR byte order, so #3B82F6 -> 0xFFF6823B.  The swatch and the
    # chart line share these values, so they stay in sync.
    if group_colors:
        group_colors[0] = [0xFFF6823B, 0xFFF9B489, 0xFFFCD3BA]
    return group_colors
...
class ColorJointModel(TableModel):
    ...
    def __init__(self, gains_tuner, value_changed_fn, **kwargs):
        ...
        # Mimic joints are excluded from plotting/selection so they never appear
        # as tunable joints. Colors stay keyed by DOF index so the plot colors
        # match the (unfiltered) DOF-indexed trajectory data.
        mimic_dof_indices = set()
        for entry in self.gains_tuner.get_joint_entries():
            try:
                if gain_tuner.is_joint_mimic(entry.joint):
                    mimic_dof_indices.add(entry.dof_index)
            except Exception:
                pass
        self._children = [
            ColorJointItem(
            ...
            if joint_index not in mimic_dof_indices
        ]
