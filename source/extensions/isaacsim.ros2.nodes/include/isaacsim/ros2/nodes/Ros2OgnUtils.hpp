...
/** @file
 * @brief OmniGraph utilities for the ROS 2 core
 * @details
                                         ...
                                         const std::string prependStr)
{
    ...
    auto dynamicMessage = std::static_pointer_cast<Ros2DynamicMessage>(message);
    auto messageFields = dynamicMessage->getMessageFields();
    bool status = dynamicMessage->isValid();
    if (!status)
    {
        CARB_LOG_ERROR("%s does not exist or is not available in the ROS 2 environment", messageType.c_str());
        message.reset();
                                     ...
                                     bool isOutput)
{
    if (!message)
    {
        CARB_LOG_ERROR("Cannot write OmniGraph data to a null ROS 2 dynamic message");
        return false;
    }
    auto dynamicMessage = std::static_pointer_cast<Ros2DynamicMessage>(message);
    if (!dynamicMessage->isValid())
    {
        CARB_LOG_ERROR("Cannot write OmniGraph data to an invalid ROS 2 dynamic message");
        return false;
    }
        ...
        case omni::fabric::BaseDataType::eUnknown:
        {
            if (messageField.isArray)
            {
                ...
                if (checkCondition(inputValue, "Unable to read message array"))
                {
                    for (size_t j = 0; j < data.size(); ++j)
                    {
                        data[j] = nlohmann::json::parse(db.tokenToString(*(*inputValue + j)), nullptr, false);
                        if (!data[j].is_object())
                        {
                            CARB_LOG_ERROR("Message array field '%s' element %zu must be a JSON object",
                                           messageField.name.c_str(), j);
                            return false;
                        }
                    }
                }
    ...
    dynamicMessage->writeData(messageData, true);
    return true;
}
                                          ...
                                          bool isOutput)
{
    if (!message)
    {
        CARB_LOG_ERROR("Cannot write a null ROS 2 dynamic message to OmniGraph attributes");
        return false;
    }
    auto dynamicMessage = std::static_pointer_cast<Ros2DynamicMessage>(message);
    if (!dynamicMessage->isValid())
    {
        CARB_LOG_ERROR("Cannot write an invalid ROS 2 dynamic message to OmniGraph attributes");
        return false;
    }

    const std::vector<std::shared_ptr<void>> messageData = dynamicMessage->readData(true);
    auto messageFields = dynamicMessage->getMessageFields();
