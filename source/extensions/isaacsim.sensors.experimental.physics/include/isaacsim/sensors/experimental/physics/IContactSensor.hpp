...
struct ContactRawData
{
    uint64_t body0{ 0 }; ///< Encoded absolute SdfPath token for the sensor's rigid body.
    uint64_t body1{ 0 }; ///< Encoded absolute SdfPath token for the other rigid body or contacted static shape.
 ...
 * @details Processes physics tensor contact data to produce filtered sensor
 * readings. Supports configurable radius filtering and force thresholds.
 *
 * The plugin is self-driving: it subscribes to physics simulation events
...
struct IContactSensor
{
     ...
     *
     * @note Side effect (PhysX only): applies PhysxSchemaPhysxContactReportAPI on
     * the parent rigid body when the active engine is not Newton.
     *
