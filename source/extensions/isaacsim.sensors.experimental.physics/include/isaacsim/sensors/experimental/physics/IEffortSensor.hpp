 ...
 * engine for a specified joint. Newton does not currently expose projected joint
 * forces, so readings are invalid when Newton is the active backend rather than
 * being reported as a semantically different actuation force or a valid zero.
 *
...
struct IEffortSensor
{
     ...
     * @param jointPrimPath USD path used when the sensor was created.
     * @return Sensor reading. isValid is false if the sensor is disabled or not found, or if the active backend does
     *         not expose projected joint forces.
     */
