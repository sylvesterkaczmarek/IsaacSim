...
struct ImuSensorReading
{
    float linearAccelerationX{ 0.0f }; ///< Linear acceleration along X axis in stage linear units per second squared.
    float linearAccelerationY{ 0.0f }; ///< Linear acceleration along Y axis in stage linear units per second squared.
    float linearAccelerationZ{ 0.0f }; ///< Linear acceleration along Z axis in stage linear units per second squared.
...
struct ImuRawData
{
    ...
    float linearVelocityX{ 0.0f }; ///< World-frame linear velocity X component in m/s.
    float linearVelocityY{ 0.0f }; ///< World-frame linear velocity Y component in m/s.
    float linearVelocityZ{ 0.0f }; ///< World-frame linear velocity Z component in m/s.
 ...
 * rigid body velocities and Pose.hpp for sensor world transforms. Supports
...
struct IImuSensor
{
     ...
     * @details The reading models an accelerometer at the body origin, so lever-arm terms for a
     * sensor offset from that origin are not modeled.
     * @param primPath USD path used when the sensor was created.
     * @param readGravity Deprecated, still honored. If true, the acceleration is specific force,
     * what an accelerometer measures (+g at rest, 0 in free fall); if false, it is the body's
     * coordinate acceleration (0 at rest, -g in free fall). g lands on whichever sensor axis
     * opposes gravity, in stage linear units per second squared rather than m/s^2.
