...
class RuleInterface(ABC):
    ...
    Implementations operate on a source ``pxr.Usd.Stage`` and may write
    opinions to a destination ``pxr.Usd.Stage``. Subclasses should record
    ...
    Stage mutation contract:
        ...
        original input must open a private ``Usd.Stage`` from
    ...
    def request_deletion(self, path: str) -> None:
        ...
        A rule must use this instead of calling ``os.remove`` directly when
        ...
        ``pxr.Sdf.Layer``) open for the duration of the rule, so the rule
