...
_log = Logger("isaacsim.core.experimental.utils.impl.backend")

_context = threading.local()  # thread-local storage to handle nested contexts and concurrent access

# Fabric Scene Delegate is a Kit application feature. Standalone (Kit-free) execution has no Fabric scene delegate, so
# it defaults to disabled. A Kit adapter may set this flag to mirror the ``/app/useFabricSceneDelegate`` setting.
_fsd_enabled = False

# Simulation-state mode for articulations. Standalone execution has no application settings, so it defaults to
# disabled. A Kit adapter may set this to mirror the ``/isaacsim/articulation/simStateMode`` setting.
_default_simstate_mode = "disabled"
...
def get_simstate_mode() -> SimStateMode:
    ...
    # Standalone (Kit-free) execution has no application settings, so the simulation-state mode defaults to disabled.
    # A Kit adapter may set ``_default_simstate_mode`` to mirror the ``/isaacsim/articulation/simStateMode`` setting.
    mode_str = _default_simstate_mode or "disabled"
    try:
        return SimStateMode(mode_str.lower())
    except ValueError:
        _log.warning(f"Invalid simStateMode '{mode_str}', falling back to 'disabled'")
...
def get_current_backend(supported_backends: list[str], *, raise_on_unsupported: bool | None = None) -> str:
    ...
    if backend not in supported_backends:
        ...
        _log.warning(f"Unsupported backend: '{backend}'. Falling back to '{supported_backends[0]}' backend.")
