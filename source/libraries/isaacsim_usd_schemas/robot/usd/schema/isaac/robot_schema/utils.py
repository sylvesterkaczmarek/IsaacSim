...
def _discover_articulation_graph(
    ...
    *,
    ...
    # Recover joints and rigid bodies from disconnected components. Converters
    # can collapse a fixed joint into a non-rigid Xform while leaving a
    # downstream joint targeted at that frame. The primary walk cannot reach
    # that component, but its joint endpoints remain valid robot links.
    for joint_prim in articulation_joints:
        joint_key = str(joint_prim.GetPath())
        ...
        ApplyJointAPI(joint_prim)
        if not joint_prim.HasAPI(Classes.JOINT_API.value):
            continue
        ordered_joints.append(joint_prim)
        visited_joints.add(joint_key)
        ...
        for body_index in (0, 1):
            ...
            owning_sub = _owning_sub_robot(body_key)
            if owning_sub is not None:
                if owning_sub not in visited_links:
                    visited_links.add(owning_sub)
                    sub_prim = stage.GetPrimAtPath(owning_sub)
            ...
            if body_key in visited_links:
                continue
            body_prim = stage.GetPrimAtPath(body_path)
            if not body_prim or not body_prim.HasAPI(pxr.UsdPhysics.RigidBodyAPI):
                continue

            ApplyLinkAPI(body_prim)
            if not body_prim.HasAPI(Classes.LINK_API.value):
                continue
            visited_links.add(body_key)
            ordered_links.append(body_prim)

            if detect_sites:
                link_sites = _detect_sites_for_link(body_prim)
                for site in link_sites:
                    ApplySiteAPI(site)
                if link_sites:
                    sites_by_link[body_key] = link_sites
