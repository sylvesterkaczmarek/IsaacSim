...
class UR10ePickPlace:
    ...
    Args:
        xrdf_dir: Directory containing a custom robot description, or None to load the supported UR10 model.
        urdf_filename: URDF filename loaded from ``xrdf_dir`` when a custom description is requested.
        xrdf_filename: XRDF filename loaded from ``xrdf_dir`` when a custom description is requested.
    ...
    def forward(self) -> bool:
        ...
        Raises:
            RuntimeError: If the RMPflow controller rejects a phase reset.
        """
        if self.is_done():
            return False
...
def main(args: argparse.Namespace, app: SimulationApp) -> None:
    ...
    Args:
        args: Parsed options controlling device, robot configuration, headless mode, and test length.
